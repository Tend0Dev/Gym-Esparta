...
export const CardMembership = ({ userData }: CardMembershipProps) => {
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <div
        ref={cardRef}
        className="w-[365px] h-60 md:w-[450px] md:h-72 rounded-xl p-6 relative overflow-hidden bg-cover bg-center"
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-2 md:mb-5">
            <div className="bg-accent p-2 rounded-lg">
              <Dumbbell className="size-4 md:size-6 text-secondary" />
            </div>
            <h1 className="text-xl md:text-3xl font-bold text-accent tracking-wider">
              ESPARTA
            </h1>
          </div>

          <p className="text-gray-100 font-semibold mb-2 md:mb-4 ml-2">{fechaTexto}</p>
          <p className="text-center text-xl md:text-2xl mb-2 font-bold text-accent uppercase">{planName || "PLAN"}</p>
          ...
          <div className="flex items-center gap-4">
            {userData.foto ? (  
              <img
                src={userData.foto}
                alt="Foto de usuario"
                className="size-18 md:size-20 rounded-lg object-cover border border-accent"
              />
            ) : (
              <div className="size-18 md:size-20 rounded-lg bg-gray-400 flex items-center justify-center text-gray-700 text-sm">
                {personIcon}
              </div>
            )}
            <div>
              <h2 className="md:text-lg font-semibold text-white">{userData.nombre || 'Nombre no disponible'}</h2>
